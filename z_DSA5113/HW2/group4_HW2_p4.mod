...
param m {ARCS}	default 1;		  # multiplier of flow on arc(i,j)
var x {ARCS} integer;                     # flow on arc (i,j)
...
sum{j in NODES: (i,j) in ARCS} x[i,j] - sum{j in NODES: (j,i) in ARCS} x[j,i]*m[j,i] = b[i];

subject to capacity {(i,j) in ARCS}: l[i,j] <= x[i,j] <= u[i,j];

data "C:\Users\Dylan\Documents\OU\Classes\DSA5113\HW2\problem_4.dat";
